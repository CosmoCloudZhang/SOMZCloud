...
def plot_deviation(sigma, rho):
    ...
    figure.colorbar(cax = cax, mappable = image, orientation = 'horizontal', label = r'$\mathcal{R}^\eta_{m m^\prime}$')
...
def main(tag, name, label, folder):
    ...
    os.makedirs(os.path.join(prior_folder, '{}/DEVIATION/'.format(tag)), exist_ok=True)
    os.makedirs(os.path.join(prior_folder, '{}/DEVIATION/{}/'.format(tag, name)), exist_ok=True)
    os.makedirs(os.path.join(prior_folder, '{}/DEVIATION/{}/{}/'.format(tag, name, label)), exist_ok=True)
    
    # Info
    with h5py.File(os.path.join(analyze_folder, '{}/VALUE/{}/{}.hdf5'.format(tag, name, label)), 'r') as file:
        
        sigma_lens = file['lens']['sigma_eta'][...]
        sigma_source = file['source']['sigma_eta'][...]
        
        rho_lens = file['lens']['rho_eta'][...]
        rho_source = file['source']['rho_eta'][...]
    ...
    # Plot Deviation
    figure = plot_deviation(sigma_lens, rho_lens)
    figure.savefig(os.path.join(prior_folder, '{}/DEVIATION/{}/{}/FIGURE_LENS.pdf'.format(tag, name, label)), format='pdf', bbox_inches='tight', dpi=512)
    pyplot.close(figure)
    
    figure = plot_deviation(sigma_source, rho_source)
    figure.savefig(os.path.join(prior_folder, '{}/DEVIATION/{}/{}/FIGURE_SOURCE.pdf'.format(tag, name, label)), format='pdf', bbox_inches='tight', dpi=512)
    pyplot.close(figure)
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Prior Deviation')
