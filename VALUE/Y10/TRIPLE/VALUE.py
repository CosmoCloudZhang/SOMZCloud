...
def main(tag, name, label, folder):
    ...
    os.makedirs(os.path.join(value_folder, '{}/TRIPLE/'.format(tag)), exist_ok = True)
    os.makedirs(os.path.join(value_folder, '{}/TRIPLE/{}/'.format(tag, name)), exist_ok = True)
    ...
    covariance = covariance[0: cell_size, 0: cell_size]
    
    # Data
    data_average = numpy.zeros((cell_size))
    mask = numpy.zeros(cell_size, dtype=bool)
    data_shift = numpy.zeros((data_size, cell_size))
    data_scale = numpy.zeros((data_size, cell_size))
    data_correct = numpy.zeros((data_size, cell_size))
    ...
    ell_maximal_ee = k_maximal_ee * pyccl.comoving_radial_distance(cosmo=cosmology, a=1 / (1 + bin_lens)) - 1 / 2
    ...
    for i in range(bin_source_size):
        for j in range(i, bin_source_size):
            ...
            index1 = cell_tt_size + cell_te_size + index * ell_size
            index2 = cell_tt_size + cell_te_size + (index + 1) * ell_size
    ...
    # Save
    with h5py.File(os.path.join(value_folder, '{}/TRIPLE/{}/{}.hdf5'.format(tag, name, label)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Value Triple')
