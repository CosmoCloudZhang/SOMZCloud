...
def main(tag, name, number, folder):
    '''
    Stack of the spectroscopic redshift distributions of the lens and source samples
    ...
    # Bin
    with h5py.File(os.path.join(summarize_folder, '{}/{}/LENS/LENS0/STACK.hdf5'.format(tag, name)), 'r') as file:
        bin_lens_size = file['meta']['bin_size'][...]
    
    with h5py.File(os.path.join(summarize_folder, '{}/{}/SOURCE/SOURCE0/STACK.hdf5'.format(tag, name)), 'r') as file:
    ...
    with h5py.File(os.path.join(synthesize_folder, '{}/{}/STACK.hdf5'.format(tag, name)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Synthesize Stack')
