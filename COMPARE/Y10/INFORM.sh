...
for INDEX in $(seq 0 $NUMBER); do
    ...
    INPUT_CONFIG="${BASE_FOLDER}COMPARE/${TAG}/INFORM/INFORM${INDEX}.yaml"
    INPUT_DATA="${BASE_FOLDER}DATASET/${TAG}/DEGRADATION/DATA${INDEX}.hdf5"
    # Run applications
    python -u "${BASE_PATH}COMPARE/${TAG}/INFORM.py" --tag=$TAG --index=$INDEX --folder=$BASE_FOLDER &&
