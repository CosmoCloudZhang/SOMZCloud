...
def main(tag, name, index, folder):
    ...
    # Target
    with h5py.File(os.path.join(constrain_folder, '{}/TARGET/DATA{}.hdf5'.format(tag, index)), 'r') as file:
        bin_lens = file['bin_lens'][...]
        application_z_phot = file['z_phot'][...]
    
    with h5py.File(os.path.join(constrain_folder, '{}/LENS/LENS{}/TARGET.hdf5'.format(tag, index)), 'r') as file:
    ...
    for m in range(bin_lens_size):
        ...
        if target_size > 0 and reference_size > 0:
            ...
            for n in range(data_size):
                ...
                # Restriction Mask
                restriction_cluster_mask = filter_data[restriction_cluster_id_data]
                restriction_weight_data = numpy.array(restriction_cluster_mask, dtype=numpy.float32)
                restriction_ensemble_indices = numpy.digitize(restriction_z_spec_data, bins=z_grid, right=False) - 1
                
                # Ensemble Cluster
                ensemble_cluster = numpy.zeros((cluster_size, grid_size + 1))
                numpy.add.at(ensemble_cluster, (restriction_cluster_id_data[restriction_cluster_mask], restriction_ensemble_indices[restriction_cluster_mask]), restriction_weight_data[restriction_cluster_mask])
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Summarize Bound DIR Lens')
