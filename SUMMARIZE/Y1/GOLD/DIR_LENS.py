...
def main(tag, name, index, folder):
    ...
    with h5py.File(os.path.join(compare_folder, '{}/LENS/LENS{}/TARGET.hdf5'.format(tag, index)), 'r') as file:
        target_lens = file['target'][...]
    
    # Degradation
    with h5py.File(os.path.join(summarize_folder, '{}/{}/ESTIMATE/ESTIMATE{}.hdf5'.format(tag, name, index)), 'r') as file:
        cell_size = file['degradation']['cell_size'][...]
        degradation_cell_id = file['degradation']['cell_id'][...]
    ...
    with h5py.File(os.path.join(compare_folder, '{}/LENS/LENS{}/REFERENCE.hdf5'.format(tag, index)), 'r') as file:
    ...
    for m in range(bin_lens_size):
        ...
        # Degradation
        degradation_z_phot_reference = degradation_z_phot[reference]
        degradation_z_spec_reference = degradation_redshift[reference]
        degradation_cell_id_reference = degradation_cell_id[reference]
        ...
        if target_size > 0 and reference_size > 0:
            ...
            for n in range(data_size):
                ...
                # Degradation
                degradation_indices = random_generator.choice(numpy.arange(reference_size), size=reference_size, replace=True)
                
                degradation_z_phot_data = degradation_z_phot_reference[degradation_indices]
                degradation_z_spec_data = degradation_z_spec_reference[degradation_indices]
                degradation_cell_id_data = degradation_cell_id_reference[degradation_indices]
                
                degradation_cluster_id_data = cluster_id[degradation_cell_id_data]
                degradation_cluster_count_data = numpy.bincount(degradation_cluster_id_data, minlength=cluster_size)
                
                degradation_cluster_z_phot_data = numpy.bincount(degradation_cluster_id_data, weights=degradation_z_phot_data, minlength=cluster_size)
                degradation_cluster_z_phot_data = numpy.divide(degradation_cluster_z_phot_data, degradation_cluster_count_data, out=numpy.zeros(cluster_size, dtype=numpy.float32), where=degradation_cluster_count_data > 0)
                
                degradation_cluster_z_spec_data = numpy.bincount(degradation_cluster_id_data, weights=degradation_z_spec_data, minlength=cluster_size)
                degradation_cluster_z_spec_data = numpy.divide(degradation_cluster_z_spec_data, degradation_cluster_count_data, out=numpy.zeros(cluster_size, dtype=numpy.float32), where=degradation_cluster_count_data > 0)
                
                # Filter
                filter_data = (application_cluster_count_data > 0) & (degradation_cluster_count_data > 0)
                
                # Degradation Mask
                degradation_cluster_mask = filter_data[degradation_cluster_id_data]
                degradation_weight_data = numpy.array(degradation_cluster_mask, dtype=numpy.float32)
                degradation_ensemble_indices = numpy.digitize(degradation_z_spec_data, bins=z_grid, right=False) - 1
                
                # Ensemble Cluster
                ensemble_cluster = numpy.zeros((cluster_size, grid_size + 1))
                numpy.add.at(ensemble_cluster, (degradation_cluster_id_data[degradation_cluster_mask], degradation_ensemble_indices[degradation_cluster_mask]), degradation_weight_data[degradation_cluster_mask])
            ...
            data_lens[m, :, :] = numpy.zeros((data_size, grid_size + 1))
    ...
    # Save
    with h5py.File(os.path.join(summarize_folder, '{}/{}/LENS/LENS{}/DIR.hdf5'.format(tag, name, index)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Summarize Gold DIR Lens')
