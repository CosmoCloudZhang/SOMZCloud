...
def main(tag, name, index, folder):
    ...
    nu_lens = numpy.zeros((bin_lens_size, data_size))
    gamma_lens = numpy.zeros((bin_lens_size, data_size))
    kappa_lens = numpy.zeros((bin_lens_size, data_size))
    lambda_lens = numpy.zeros((bin_lens_size, data_size))
        ...
        if target_size > 0 and reference_size > 0:
            ...
            for n in range(data_size):
                ...
                if numpy.sum(filter_data) > 0:
                    nu_lens[m, n] = numpy.sum(filter_data)
            ...
            nu_lens[m, :] = numpy.zeros(data_size)
            gamma_lens[m, :] = numpy.zeros(data_size)
            kappa_lens[m, :] = numpy.zeros(data_size)
            lambda_lens[m, :] = numpy.zeros(data_size)
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Summarize Gold Truth Lens')
