...
def main(tag, number, folder):
    ...
    for m in range(bin_size):
        plot[m, 1].plot([0.0, 0.6], [factor_source, factor_source], color='black', linestyle='--')
        
        plot[m, 1].scatter(metric_source[m, :], som_delta_source[m, :], color='darkblue', marker='o', s=10)
        
        plot[m, 1].scatter(metric_source[m, :], model_delta_source[m, :], color='darkgreen', marker='o', s=10)
        
        plot[m, 1].scatter(metric_source[m, :], product_delta_source[m, :], color='darkorange', marker='o', s=10)
