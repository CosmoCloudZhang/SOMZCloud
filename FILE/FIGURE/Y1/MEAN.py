...
def main(tag, folder):
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].hist(histogram_mean_source[:, m], bins=size, range=(histogram_center_source[m] - source_shift[m], histogram_center_source[m] + source_shift[m]), color='darkblue', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(fzb_mean_source[:, m], bins=size, range=(histogram_center_source[m] - source_shift[m], histogram_center_source[m] + source_shift[m]), color='darkorange', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(som_mean_source[:, m], bins=size, range=(histogram_center_source[m] - source_shift[m], histogram_center_source[m] + source_shift[m]), color='darkgreen', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].text(histogram_center_source[m] - 0.8 * source_shift[m], 120, r'$\delta_{\langle z \rangle}^\mathrm{FZB} =$' + r'${:.3f}$'.format(fzb_delta_source[m]), color='darkorange', fontsize=20)
        
        plot[m, 1].text(histogram_center_source[m] + 0.2 * source_shift[m], 120, r'$\delta_{\langle z \rangle}^\mathrm{SOM} =$' + r'${:.3f}$'.format(som_delta_source[m]), color='darkgreen', fontsize=20)
        
        plot[m, 1].set_ylim(0, 150)
        plot[m, 1].set_yticklabels([])
        plot[m, 1].set_xlim(histogram_center_source[m] - source_shift[m], histogram_center_source[m] + source_shift[m])
