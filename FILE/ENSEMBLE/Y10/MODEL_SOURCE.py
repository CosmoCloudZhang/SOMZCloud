...
def main(tag, number, folder):
    ...
    for n in range(number):
        ...
        with h5py.File(os.path.join(summarize_folder, '{}/SOURCE/SOURCE{}/MODEL.hdf5'.format(tag, n + 1)), 'r') as file:
    ...
    weight_source = numpy.square(scipy.stats.mstats.gmean(fraction_source, axis=1) / metric[:, numpy.newaxis])
    
    # Ensemble Data
    count = 32
    ensemble_size = 500000
    with multiprocessing.Pool(processes=count) as pool:
        ensemble_data = numpy.stack(pool.starmap(ensemble, [(data_source, weight_source, z_grid, number, sample_size) for _ in range(ensemble_size)]), axis=0)
    ...
    with h5py.File(os.path.join(ensemble_folder, '{}/SOURCE/MODEL.hdf5'.format(tag)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Ensemble Model')
