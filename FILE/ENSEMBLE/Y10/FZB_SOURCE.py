...
def main(tag, number, folder):
    '''
    FZB ensemble of the source samples
    ...
    for n in range(number):
        ...
        with h5py.File(os.path.join(fzb_folder, '{}/SOURCE/SOURCE{}/SUMMARIZE.hdf5'.format(tag, n + 1)), 'r') as file:
            data[n, :, :, :] = file['sample'][...]
    ...
    with h5py.File(os.path.join(ensemble_folder, '{}/SOURCE/FZB.hdf5'.format(tag)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Ensemble FZB')
