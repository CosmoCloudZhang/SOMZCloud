...
def main(path, size, width, length):
    ...
    for n in range(length):
        for m in range(size):
            sample_name = os.path.join(data_path, 'SOM/SOURCE/SOURCE{}/SOM_SUMMARIZE{}.hdf5'.format(n + 1, m + 1))
    ...
    # Save
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/SOURCE/SOM_ENSEMBLE_SELECT_SAMPLE.hdf5'), 'w') as file:
    ...
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/SOURCE/SOM_ENSEMBLE_SELECT_MEAN.hdf5'), 'w') as file:
