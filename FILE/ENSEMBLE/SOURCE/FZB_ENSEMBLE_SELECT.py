...
def main(path, size, width, length):
    ...
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/SOURCE/FZB_ENSEMBLE_SELECT_SAMPLE.hdf5'), 'w') as file:
        file.create_dataset('sample', data=ensemble_sample, dtype=numpy.float64)
        file.create_dataset('z', data=z_grid, dtype=numpy.float64)
    
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/SOURCE/FZB_ENSEMBLE_SELECT_MEAN.hdf5'), 'w') as file:
