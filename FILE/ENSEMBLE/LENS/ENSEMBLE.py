...
def main(path, size, width, length):
    ...
    for n in range(length):
        for m in range(size):
            sample_name = os.path.join(data_path, 'ENSEMBLE/LENS/LENS{}/SUMMARIZE{}.hdf5'.format(n + 1, m + 1))
    ...
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/LENS/ENSEMBLE_SAMPLE.hdf5'), 'w') as file:
        file.create_dataset('sample', data=ensemble_sample, dtype=numpy.float64)
        file.create_dataset('z', data=z_grid, dtype=numpy.float64)

    with h5py.File(os.path.join(data_path, 'ENSEMBLE/LENS/ENSEMBLE_MEAN.hdf5'), 'w') as file:
