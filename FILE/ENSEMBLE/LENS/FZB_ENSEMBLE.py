...
def main(path, size, width, length):
    ...
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/LENS/FZB_ENSEMBLE_SAMPLE.hdf5'), 'w') as file:
        file.create_dataset('sample', data=ensemble_sample, dtype=numpy.float64)
        file.create_dataset('z', data=z_grid, dtype=numpy.float64)
    
    with h5py.File(os.path.join(data_path, 'ENSEMBLE/LENS/FZB_ENSEMBLE_MEAN.hdf5'), 'w') as file:
