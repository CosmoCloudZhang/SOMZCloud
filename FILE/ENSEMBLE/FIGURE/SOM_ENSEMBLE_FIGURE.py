...
def main(path):
    ...
    for m in range(bin_size):
        ...
        plot[m, 0].text(lens_mean_data_select[m] + 0.5 * shift1, 150, r'$\Delta \langle z \rangle = {:.3f}$'.format(lens_mean_data[m] - lens_mean_data_select[m]), fontsize=20, ha='center')
        
        plot[m, 0].set_ylim(0, 250)
        ...
        plot[m, 1].text(source_mean_data_select[m] + 0.5 * shift1, 150, r'$\Delta \langle z \rangle = {:.3f}$'.format(source_mean_data[m] - source_mean_data_select[m]), fontsize=20, ha='center')
        
        plot[m, 1].set_ylim(0, 250)
        plot[m, 1].set_yticklabels([])
        plot[m, 1].set_xlim(source_mean_data_select[m] + shift1, source_mean_data_select[m] + shift2)
    
    plot[0, 0].set_title(r'$\mathrm{Lens}$')
    plot[0, 1].set_title(r'$\mathrm{Source}$')
