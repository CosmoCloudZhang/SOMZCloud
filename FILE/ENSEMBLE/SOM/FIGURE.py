...
def main(folder):
    ...
    # Ensemble
    with h5py.File(os.path.join(ensemble_folder, 'LENS/SOM_SELECT.hdf5'), 'r') as file:
        lens_select = file['ensemble'][:].astype(numpy.float32)
    
    with h5py.File(os.path.join(ensemble_folder, 'SOURCE/SOM_SELECT.hdf5'), 'r') as file:
        source_select = file['ensemble'][:].astype(numpy.float32)
    
    with h5py.File(os.path.join(ensemble_folder, 'LENS/SOM_ENSEMBLE.hdf5'), 'r') as file:
        lens_ensemble = file['ensemble'][:].astype(numpy.float32)
    
    with h5py.File(os.path.join(ensemble_folder, 'SOURCE/SOM_ENSEMBLE.hdf5'), 'r') as file:
    ...
    figure.savefig(os.path.join(ensemble_folder, 'FIGURE/SOM_FIGURE.png'), bbox_inches='tight', dpi=512)
