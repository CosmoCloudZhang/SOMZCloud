...
def save_mean(z_lens, z_mean, z_source, bin_lens, bin_source, mag0_lens, mag_source, width):
    """
    ...
    lens_mean = numpy.zeros((width, lens_size, grid_size), dtype=numpy.float32)
    
    for n in range(width):
        for m in range(lens_size):
            select = select_lens & (bin_lens[m] < z_mean) & (z_mean < bin_lens[m + 1])
            z_data = numpy.random.choice(z_mean[select], z_mean[select].size, replace=True)
            lens_mean[n, m, :] = numpy.histogram(z_data, bins=z_source, range=(z1_source, z2_source), density=False)[0].astype(numpy.float32)
    
    lens_count = numpy.sum(lens_mean, axis=2)
    lens = {'data': lens_mean, 'count': lens_count}
    
    # Source
    grid_size = z_source.size - 1
    source_size = len(bin_source) - 1
    source_mean = numpy.zeros((width, source_size, grid_size), dtype=numpy.float32)
    
    for n in range(width):
        for m in range(source_size):
            select = select_source & (bin_source[m] < z_mean) & (z_mean < bin_source[m + 1])
            z_data = numpy.random.choice(z_mean[select], z_mean[select].size, replace=True)
            source_mean[n, m, :] = numpy.histogram(z_data, bins=z_source, range=(z1_source, z2_source), density=False)[0].astype(numpy.float32)
    
    source_count = numpy.sum(source_mean, axis=2)
...
def main(path, index):
    ...
    mag0_lens = 24.1
    # Save mean
    lens_mean, source_mean = save_mean(z_lens, z_mean, z_source, bin_lens, bin_source, mag0_lens, mag_source, width)
    
    with h5py.File(os.path.join(data_path, 'LENS/LENS{}/SELECT_MEAN.hdf5'.format(index)), 'w') as file:
        for key, value in lens_mean.items():
            file.create_dataset(key, data=value)
    
    with h5py.File(os.path.join(data_path, 'SOURCE/SOURCE{}/SELECT_MEAN.hdf5'.format(index)), 'w') as file:
