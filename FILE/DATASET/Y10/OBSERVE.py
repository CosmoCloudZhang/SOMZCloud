...
def main(tag, folder):
    ...
    for value in observation_list:
        ...
        observation_catalog = error_model(pandas.DataFrame(catalog), random_state=seed)
        
        flux0 = 3631e6
        flux1 = flux0 * numpy.power(10, -0.4 * catalog['mag_r_lsst'])
        flux2 = flux0 * numpy.power(10, -0.4 * catalog['mag_i_lsst'])
        
        error1 = 2.5 / numpy.log(10) * observation_catalog['mag_r_lsst_err'].values * flux1 * numpy.sqrt(exposure['mag_r_lsst'])
        error2 = 2.5 / numpy.log(10) * observation_catalog['mag_i_lsst_err'].values * flux2 * numpy.sqrt(exposure['mag_i_lsst'])
        ...
        for key in catalog.keys():
            if key in observation.keys():
                observation[key] = numpy.concatenate([observation[key], catalog[key][filter]])
            else:
                observation[key] = catalog[key][filter]
