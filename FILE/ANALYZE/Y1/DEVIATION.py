...
def main(tag, folder):
    ...
    for label in label_list:
        ...
        for m in range(bin_size):
            ...
            plot[m, 1].text(x=histogram_scatter_source[m] - range_source[m] * 0.80, y=250, s=r'$\delta^\mathrm{SOM}_{\bar{\eta}} = ' + r'{:.3f}$'.format(som_delta_source[m]), fontsize=15, color='darkblue')
            
            plot[m, 1].text(x=histogram_scatter_source[m] - range_source[m] * 0.80, y=100, s=r'$\delta^\mathrm{Model}_{\bar{\eta}} = ' + r'{:.3f}$'.format(model_delta_source[m]), fontsize=15, color='darkgreen')
            
            plot[m, 1].text(x=histogram_scatter_source[m] + range_source[m] * 0.24, y=250, s=r'$\delta^\mathrm{Product}_{\bar{\eta}} = ' + r'{:.3f}$'.format(product_delta_source[m]), fontsize=15, color='darkorange')
            
            plot[m, 1].text(x=histogram_scatter_source[m] + range_source[m] * 0.24, y=100, s=r'$\delta^\mathrm{Fiducial}_{\bar{\eta}} = ' + r'{:.3f}$'.format(fiducial_delta_source[m]), fontsize=15, color='darkred')
