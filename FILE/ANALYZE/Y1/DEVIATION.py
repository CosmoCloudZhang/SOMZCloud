...
def main(tag, folder):
    ...
    for label in label_list:
        ...
        size = 100
        bin_size = 5
        lens_range = 0.03 * histogram_scatter_lens
        source_range = 0.06 * histogram_scatter_source
        ...
        for m in range(bin_size):
            ...
            plot[m, 1].text(x=histogram_scatter_source[m] - source_range[m] * 0.9, y=250, s=r'$\delta^\mathrm{SOM}_{\bar{\langle \varsigma \rangle}} = ' + r'{:.3f}$'.format(som_delta_source[m]), fontsize=15, color='darkblue')
            
            plot[m, 1].text(x=histogram_scatter_source[m] - source_range[m] * 0.9, y=100, s=r'$\delta^\mathrm{Model}_{\bar{\langle \varsigma \rangle}} = ' + r'{:.3f}$'.format(model_delta_source[m]), fontsize=15, color='darkgreen')
            
            plot[m, 1].text(x=histogram_scatter_source[m] + source_range[m] * 0.1, y=250, s=r'$\delta^\mathrm{Product}_{\bar{\langle \varsigma \rangle}} = ' + r'{:.3f}$'.format(product_delta_source[m]), fontsize=15, color='darkorange')
            
            plot[m, 1].text(x=histogram_scatter_source[m] + source_range[m] * 0.1, y=100, s=r'$\delta^\mathrm{Fiducial}_{\bar{\langle \varsigma \rangle}} = ' + r'{:.3f}$'.format(fiducial_delta_source[m]), fontsize=15, color='darkred')
