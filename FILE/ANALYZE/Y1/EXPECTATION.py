...
def main(tag, folder):
    ...
    for label in label_list:
        ...
        for m in range(bin_size):
            plot[m, 1].hist(som_expectation_source[:, m], bins=size, range=(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m]), color='darkblue', density=True, histtype='step', linewidth=2.0, linestyle='-')
            
            plot[m, 1].hist(model_expectation_source[:, m], bins=size, range=(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m]), color='darkgreen', density=True, histtype='step', linewidth=2.0, linestyle='-')
            
            plot[m, 1].hist(product_expectation_source[:, m], bins=size, range=(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m]), color='darkorange', density=True, histtype='step', linewidth=2.0, linestyle='-')
            
            plot[m, 1].hist(fiducial_expectation_source[:, m], bins=size, range=(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m]), color='darkred', density=True, histtype='step', linewidth=2.0, linestyle='-')
            
            plot[m, 1].hist(histogram_expectation_source[:, m], bins=size, range=(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m]), color='black', density=True, histtype='step', linewidth=2.0, linestyle='-')
            
            plot[m, 1].text(x=histogram_middle_source[m] - range_source[m] * 0.80, y=500, s=r'$\delta^\mathrm{SOM}_{\bar{\mu}} = ' + r'{:.3f}$'.format(som_delta_source[m]), fontsize=15, color='darkblue')
            
            plot[m, 1].text(x=histogram_middle_source[m] - range_source[m] * 0.80, y=200, s=r'$\delta^\mathrm{Model}_{\bar{\mu}} = ' + r'{:.3f}$'.format(model_delta_source[m]), fontsize=15, color='darkgreen')
            
            plot[m, 1].text(x=histogram_middle_source[m] + range_source[m] * 0.24, y=500, s=r'$\delta^\mathrm{Product}_{\bar{\mu}} = ' + r'{:.3f}$'.format(product_delta_source[m]), fontsize=15, color='darkorange')
            
            plot[m, 1].text(x=histogram_middle_source[m] + range_source[m] * 0.24, y=200, s=r'$\delta^\mathrm{Fiducial}_{\bar{\mu}} = ' + r'{:.3f}$'.format(fiducial_delta_source[m]), fontsize=15, color='darkred')
            
            plot[m, 1].fill_betweenx(y=[10, 800], x1=histogram_middle_source[m] - factor_source[m], x2=histogram_middle_source[m] + factor_source[m], color='gray', alpha=0.5)
            
            plot[m, 1].set_ylim(10, 800)
            plot[m, 1].set_xlim(histogram_middle_source[m] - range_source[m], histogram_middle_source[m] + range_source[m])
