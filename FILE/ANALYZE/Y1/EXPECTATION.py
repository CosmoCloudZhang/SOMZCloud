...
def main(tag, label, folder):
    ...
    for m in range(bin_size):
        plot[m, 1].hist(histogram_expectation_source[:, m], bins=size, range=(target_middle_source[m] - range_source[m], target_middle_source[m] + range_source[m]), color='darkblue', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(model_expectation_source[:, m], bins=size, range=(target_middle_source[m] - range_source[m], target_middle_source[m] + range_source[m]), color='darkgreen', density=True, histtype='step', linewidth=2.0, linestyle='-')
        ...
        plot[m, 1].text(x=target_middle_source[m] - range_source[m] * 0.80, y=500, s=r'$\delta^\mathtt{histogram}_{\tilde{\mu}} = ' + r'{:.3f}$'.format(histogram_delta_source[m]), fontsize=15, color='darkblue')
        
        plot[m, 1].text(x=target_middle_source[m] - range_source[m] * 0.80, y=200, s=r'$\delta^\mathtt{model}_{\tilde{\mu}} = ' + r'{:.3f}$'.format(model_delta_source[m]), fontsize=15, color='darkgreen')
        
        plot[m, 1].text(x=target_middle_source[m] + range_source[m] * 0.24, y=500, s=r'$\delta^\mathtt{product}_{\tilde{\mu}} = ' + r'{:.3f}$'.format(product_delta_source[m]), fontsize=15, color='darkorange')
        
        plot[m, 1].text(x=target_middle_source[m] + range_source[m] * 0.24, y=200, s=r'$\delta^\mathtt{fiducial}_{\tilde{\mu}} = ' + r'{:.3f}$'.format(fiducial_delta_source[m]), fontsize=15, color='darkred')
