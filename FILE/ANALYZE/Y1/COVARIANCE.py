...
def main(tag, folder):
    ...
    for label in label_list:
        ...
        for m in range(bin_lens_size):
            som_select_lens[:, m, :numpy.minimum(map_lens_size, z_size - index_lower_lens[m])] = som_data_lens[:, m, index_lower_lens[m]: numpy.minimum(z_size, index_upper_lens[m])]
            model_select_lens[:, m, :numpy.minimum(map_lens_size, z_size - index_lower_lens[m])] = model_data_lens[:, m, index_lower_lens[m]: numpy.minimum(z_size, index_upper_lens[m])]
            product_select_lens[:, m, :numpy.minimum(map_lens_size, z_size - index_lower_lens[m])] = product_data_lens[:, m, index_lower_lens[m]: numpy.minimum(z_size, index_upper_lens[m])]
            fiducial_select_lens[:, m, :numpy.minimum(map_lens_size, z_size - index_lower_lens[m])] = fiducial_data_lens[:, m, index_lower_lens[m]: numpy.minimum(z_size, index_upper_lens[m])]
            histogram_select_lens[:, m, :numpy.minimum(map_lens_size, z_size - index_lower_lens[m])] = histogram_data_lens[:, m, index_lower_lens[m]: numpy.minimum(z_size, index_upper_lens[m])]
        
        for m in range(bin_source_size):
            som_select_source[:, m, :numpy.minimum(map_source_size, z_size - index_lower_source[m])] = som_data_source[:, m, index_lower_source[m]: numpy.minimum(z_size, index_upper_source[m])]
            model_select_source[:, m, :numpy.minimum(map_source_size, z_size - index_lower_source[m])] = model_data_source[:, m, index_lower_source[m]: numpy.minimum(z_size, index_upper_source[m])]
            product_select_source[:, m, :numpy.minimum(map_source_size, z_size - index_lower_source[m])] = product_data_source[:, m, index_lower_source[m]: numpy.minimum(z_size, index_upper_source[m])]
            fiducial_select_source[:, m, :numpy.minimum(map_source_size, z_size - index_lower_source[m])] = fiducial_data_source[:, m, index_lower_source[m]: numpy.minimum(z_size, index_upper_source[m])]
            histogram_select_source[:, m, :numpy.minimum(map_source_size, z_size - index_lower_source[m])] = histogram_data_source[:, m, index_lower_source[m]: numpy.minimum(z_size, index_upper_source[m])]
        ...
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_SOM_LENS.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_source_size, map_source_size, som_covariance_source)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_SOM_SOURCE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_lens_size, map_lens_size, model_covariance_lens)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_MODEL_LENS.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_source_size, map_source_size, model_covariance_source)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_MODEL_SOURCE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_lens_size, map_lens_size, product_covariance_lens)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_PRODUCT_LENS.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_source_size, map_source_size, product_covariance_source)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_PRODUCT_SOURCE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_lens_size, map_lens_size, fiducial_covariance_lens)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_FIDUCIAL_LENS.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_source_size, map_source_size, fiducial_covariance_source)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_FIDUCIAL_SOURCE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_lens_size, map_lens_size, histogram_covariance_lens)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_HISTOGRAM_LENS.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
        
        figure = plot_covariance(bin_source_size, map_source_size, histogram_covariance_source)
        figure.savefig(os.path.join(analyze_folder, '{}/COVARIANCE/{}/FIGURE_HISTOGRAM_SOURCE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight')
        pyplot.close(figure)
