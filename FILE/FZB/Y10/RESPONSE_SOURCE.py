...
def main(index, folder):
    ...
    for m in range(len(bin_source) - 1):
        ...
        # Summarize
        summarize_single = numpy.mean(z_pdf[select_source_bin, :], axis=0)
        ...
        # Single
        histogram = numpy.histogram(z_true[select_source_bin], bins=z_grid, range=(z_grid.min(), z_grid.max()), density=True)[0]
        ...
        with h5py.File(os.path.join(fzb_folder, 'SOURCE/SOURCE{}/RESPONSE{}.hdf5'.format(index, m + 1)), 'w') as file:
