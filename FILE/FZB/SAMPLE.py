...
def main(number, folder):
    ...
    Arguments:
        count (int): The count of the processes.
    ...
    for index in range(number):
        ...
        os.makedirs(os.path.join(fzb_folder, 'LENS/LENS{}'.format(index + 1)), exist_ok=True)
        os.makedirs(os.path.join(fzb_folder, 'SOURCE/SOURCE{}'.format(index + 1)), exist_ok=True)
        ...
        estimate_name = os.path.join(dataset_folder, 'ESTIMATE{}.hdf5'.format(index + 1))
        ...
        with h5py.File(os.path.join(fzb_folder, 'LENS/BIN.hdf5'), 'r') as file:
            bin_lens = file['bin'][index, :].astype(numpy.float32)
        ...
        for m in range(len(bin_lens) - 1):
            ...
            with h5py.File(os.path.join(fzb_folder, 'LENS/LENS{}/SELECT{}.hdf5'.format(index + 1, m + 1)), 'w') as file:    
        ...
        for m in range(len(bin_source) - 1):
            ...
            with h5py.File(os.path.join(fzb_folder, 'SOURCE/SOURCE{}/SELECT{}.hdf5'.format(index + 1, m + 1)), 'w') as file:
                file.create_group(name='meta')
                file.create_group(name='data')
