...
for INDEX in $(seq 1 $NUMBER); do
    ...
    CONFIG_PATH="${BASE_FOLDER}FZB/REFERENCE/REFERENCE${INDEX}.yaml"
    OUTPUT_PATH="${BASE_FOLDER}FZB/REFERENCE/REFERENCE${INDEX}.hdf5"
    INPUT_PATH="${BASE_FOLDER}DATASET/COMBINATION/DATA${INDEX}.hdf5"
    # Run applications
    python -u "${BASE_PATH}FILE/FZB/REFERENCE.py" --index=$INDEX --folder=$BASE_FOLDER &
