...
def summarize(index, folder):
    ...
    for m in range(len(bin_lens) - 1):
        # Select
        select_name = os.path.join(fzb_folder, 'LENS/LENS{}/SELECT{}.hdf5'.format(index + 1, m + 1))
        ...
        # Save the single
        with h5py.File(os.path.join(fzb_folder, 'LENS/LENS{}/SINGLE{}.hdf5'.format(index + 1, m + 1)), 'w') as file:
        ...
        # Save the data
        with h5py.File(os.path.join(fzb_folder, 'LENS/LENS{}/SUMMARIZE{}.hdf5'.format(index + 1, m + 1)), 'w') as file:
            file.create_group('meta')
            file.create_group('data')
