...
def main(folder):
    ...
    # Ensemble
    with h5py.File(os.path.join(fzb_folder, 'LENS/ENSEMBLE.hdf5'), 'r') as file:
        lens_data = file['data'][:].astype(numpy.float32)
        lens_select = file['select'][:].astype(numpy.float32)
    
    with h5py.File(os.path.join(fzb_folder, 'SOURCE/ENSEMBLE.hdf5'), 'r') as file:
    ...
    figure.savefig(os.path.join(fzb_folder, 'FIGURE/ENSEMBLE/FIGURE.png'), bbox_inches='tight', dpi=512)
