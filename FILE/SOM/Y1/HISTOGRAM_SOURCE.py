...
def main(tag, index, folder):
    ...
    for m in range(bin_source_size):
        ...
        # Weight
        weight = numpy.array(numpy.isin(application_cell_id_select, combination_cell_id_reference), dtype=numpy.float32) / numpy.square(application_sigma_select)
        ...
        for n in range(sample_size):
            ...
            # Weight
            weight_sample = numpy.array(numpy.isin(application_cell_id_sample, combination_cell_id_sample), dtype=numpy.float32) / numpy.square(application_sigma_sample)
    ...
    # Save
    with h5py.File(os.path.join(som_folder, '{}/SOURCE/SOURCE{}/HISTOGRAM.hdf5'.format(tag, index)), 'w') as file:
