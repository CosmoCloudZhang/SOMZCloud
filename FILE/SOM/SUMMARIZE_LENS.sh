...
for BIN in $(seq 1 $SIZE); do
    for INDEX in $(seq 1 $NUMBER); do
        ...
        INPUT_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/SAMPLE${BIN}.hdf5"
        CONFIG_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/SUMMARIZE${BIN}.yaml"
        # Set path of output variables
        CELL_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/CELL${BIN}.hdf5"
        SINGLE_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/SINGLE${BIN}.hdf5"
        CLUSTER_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/CLUSTER${BIN}.hdf5"
        OUTPUT_PATH="${BASE_FOLDER}SOM/LENS/LENS${INDEX}/SUMMARIZE${BIN}.hdf5"
        # Run applications
        python -u "${BASE_PATH}/FILE/SOM/SUMMARIZE_LENS.py" --bin=$BIN --index=$INDEX --folder=$BASE_FOLDER &
