...
for BIN in $(seq 1 $WIDTH); do
    for INDEX in $(seq 1 $LENGTH); do
        ...
        CONFIG_PATH="${BASE_PATH}/DATA/SOM/SOM_SUMMARIZE_LENS${INDEX}.yaml"
        INPUT_PATH="${BASE_PATH}/DATA/SELECT/LENS/LENS${INDEX}/SELECT${BIN}.hdf5"
        # Set path of output variables
        SINGLE_PATH="${BASE_PATH}/DATA/SELECT/LENS/LENS${INDEX}/SOM_SINGLE${BIN}.hdf5"
        CELLID_PATH="${BASE_PATH}/DATA/SELECT/LENS/LENS${INDEX}/SOM_CELLID${BIN}.hdf5"
        OUTPUT_PATH="${BASE_PATH}/DATA/SELECT/LENS/LENS${INDEX}/SOM_SUMMARIZE${BIN}.hdf5"
        CLUSTER_PATH="${BASE_PATH}/DATA/SELECT/LENS/LENS${INDEX}/SOM_CELL_FILE${BIN}.hdf5"
        # Run applications
        python "${BASE_PATH}/FILE/SOM/SOM_SUMMARIZE_LENS.py" --path="${BASE_PATH}" --index=$INDEX &
