...
def main(tag, folder):
    ...
    for m in range(bin_size):
        plot[m, 1].plot(z_grid, som_source[m, :], color='darkblue', linewidth=2.0, linestyle='-')
        
        plot[m, 1].plot(z_grid, model_source[m, :], color='darkgreen', linewidth=2.0, linestyle='-')
        
        plot[m, 1].plot(z_grid, product_source[m, :], color='darkorange', linewidth=2.0, linestyle='-')
        
        plot[m, 1].plot(z_grid, histogram_source[m, :], color='black', linewidth=2.0, linestyle='-')
        ...
        plot[m, 1].text(x=numpy.minimum(numpy.maximum(z1, center_source[m] - 0.5) + 1.0, z2) - 0.2, y=6.0, s=r'$\mathrm{Bin} \, ' + r'{}$'.format(m + 1), fontsize=20)
        
        if m == 0:
            plot[m, 1].set_title(r'$\mathrm{Source}$')
    
    os.makedirs(analysis_folder, exist_ok=True)
    os.makedirs(os.path.join(analysis_folder, '{}/MARGINAL/'.format(tag)), exist_ok=True)
    
    figure.subplots_adjust(wspace=0.2, hspace=0.2)
    figure.savefig(os.path.join(analysis_folder, '{}/MARGINAL/FIGURE.pdf'.format(tag)), format='pdf', bbox_inches='tight')
