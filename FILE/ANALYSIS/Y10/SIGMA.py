...
def main(tag, folder):
    ...
    os.makedirs(os.path.join(analysis_folder, '{}/'.format(tag)), exist_ok=True)
    os.makedirs(os.path.join(analysis_folder, '{}/SIGMA/'.format(tag)), exist_ok=True)
    
    figure.subplots_adjust(right=0.80)
    figure.savefig(os.path.join(analysis_folder, '{}/SIGMA/FIGURE.pdf'.format(tag)), format='pdf', bbox_inches='tight')
    pyplot.close(figure)
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Analysis Sigma')
