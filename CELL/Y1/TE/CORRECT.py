...
def main(tag, name, label, folder):
    ...
    os.makedirs(os.path.join(cell_folder, '{}/TE/'.format(tag)), exist_ok = True)
    os.makedirs(os.path.join(cell_folder, '{}/TE/CORRECT/'.format(tag)), exist_ok = True)
    os.makedirs(os.path.join(cell_folder, '{}/TE/CORRECT/{}/'.format(tag, name)), exist_ok = True)
    ...
    # Cell Average
    cell_average = numpy.zeros((bin_lens_size, bin_source_size, ell_size))
    ...
    # Save
    with h5py.File(os.path.join(cell_folder, '{}/TE/CORRECT/{}/{}.hdf5'.format(tag, name, label)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Cell TE Correct')
