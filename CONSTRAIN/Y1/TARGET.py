...
def main(tag, index, folder):
    ...
    os.makedirs(os.path.join(constrain_folder, '{}/'.format(tag)), exist_ok=True)
    os.makedirs(os.path.join(constrain_folder, '{}/TARGET/'.format(tag)), exist_ok=True)
    ...
    for m in range(application_size // chunk_size + 1):
        ...
        if begin < stop:
            z_pdf = numpy.maximum(interpolate.CubicSpline(x=z_grid, y=estimator['data']['yvals'][begin: stop].astype(numpy.float32), axis=1, bc_type='natural', extrapolate=False)(z_mesh), 0.0)
    ...
    for m in range(average_size_lens):
        ...
        if numpy.sum(target_lens_average) > 0:
            ...
            sigma_lens[m] = stats.median_abs_deviation(bias_lens_target)
    ...
    for m in range(average_size_source):
        ...
        if numpy.sum(target_source_average) > 0:
            ...
            sigma_source[m] = stats.median_abs_deviation(bias_source_target)
    ...
    # Save
    with h5py.File(os.path.join(constrain_folder, '{}/TARGET/DATA{}.hdf5'.format(tag, index)), 'w') as file:
    ...
    with h5py.File(os.path.join(constrain_folder, '{}/LENS/LENS{}/TARGET.hdf5'.format(tag, index)), 'w') as file:    
    ...
    with h5py.File(os.path.join(constrain_folder, '{}/SOURCE/SOURCE{}/TARGET.hdf5'.format(tag, index)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Comparison Target')
