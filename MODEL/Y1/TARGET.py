...
def main(tag, index, folder):
    ...
    os.makedirs(os.path.join(model_folder, '{}/TARGET/'.format(tag)), exist_ok=True)
    ...
    # Save
    with h5py.File(os.path.join(model_folder, '{}/TARGET/DATA{}.hdf5'.format(tag, index)), 'w') as file:
    ...
    with h5py.File(os.path.join(model_folder, '{}/LENS/LENS{}/TARGET.hdf5'.format(tag, index)), 'w') as file:    
    ...
    with h5py.File(os.path.join(model_folder, '{}/SOURCE/SOURCE{}/TARGET.hdf5'.format(tag, index)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Model Target')
