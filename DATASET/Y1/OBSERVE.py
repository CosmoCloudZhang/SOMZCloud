...
def main(tag, index, folder):
    ...
    Arguments:
        ...
        index (int): The index of the observation datasets
        folder (str): The base folder of the observation datasets
    ...
    with open(os.path.join(dataset_folder, 'CATALOG/OBSERVE.yaml'), 'r') as file:
        ...
        observation_list = numpy.sort(random_generator.choice(observation_list, len(observation_list) // 2, replace=False))
    ...
    for value in observation_list:
        ...
        mu = 10
        eta = 0.1
        condition = condition & (catalog['mu'] > mu) & (catalog['eta'] > eta)
    ...
    # Save
    with h5py.File(os.path.join(dataset_folder, '{}/OBSERVATION/DATA{}.hdf5'.format(tag, index)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='Observation Datasets')
    PARSE.add_argument('--tag', type=str, required=True, help='The tag of the configuration')
    PARSE.add_argument('--index', type=int, required=True, help='The index of the observation datasets')
