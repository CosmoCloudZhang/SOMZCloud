...
def main(tag, name, folder):
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].axvspan(average_mu_source[m] - factor_source[m], average_mu_source[m] + factor_source[m], alpha=0.3, color='gray')
        plot[m, 1].text(x=average_mu_source[m] + range_source[m] / 3 * 2 * (1 + average_mu_source[m]), y=2.25, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + 1), color='black', ha='center')
        
        plot[m, 1].set_ylim(0.5, 4.5)
        plot[m, 1].tick_params(axis='x', labelsize=25)
        plot[m, 1].set_xlim(average_mu_source[m] - range_source[m] * (1 + average_mu_source[m]), average_mu_source[m] + range_source[m] * (1 + average_mu_source[m]))
