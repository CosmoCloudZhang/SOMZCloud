...
def main(tag, label, folder):
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].axhspan(-factor_lens, +factor_lens, alpha=0.3, color='gray', rasterized=True)
        plot[m, 1].text(x=5.5, y=range_lens[m] / 3 * 2, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + 1), color='black', ha='center')
        
        plot[m, 1].set_xlim(0.5, 6.5)
        plot[m, 1].set_ylim(-range_lens[m], +range_lens[m])
    ...
    for m in range(bin_size):
        ...
        plot[m, 2].axhspan(-factor_source, +factor_source, alpha=0.3, color='gray', rasterized=True)
    ...
    figure.savefig(os.path.join(analyze_folder, '{}/WIDTH/{}/FIGURE.pdf'.format(tag, label)), format='pdf', bbox_inches='tight', dpi=512)
    pyplot.close(figure)
