...
def main(tag, name, folder):
    ...
    dir_delta_lens = (dir_mu_lens - truth_mu_lens) / (1 + truth_mu_lens)
    dir_delta_source = (dir_mu_source - truth_mu_source) / (1 + truth_mu_source)
    ...
    stack_delta_lens = (stack_mu_lens - truth_mu_lens) / (1 + truth_mu_lens)
    stack_delta_source = (stack_mu_source - truth_mu_source) / (1 + truth_mu_source)
    ...
    hybrid_delta_lens = (hybrid_mu_lens - truth_mu_lens) / (1 + truth_mu_lens)
    hybrid_delta_source = (hybrid_mu_source - truth_mu_source) / (1 + truth_mu_source)
    ...
    for m in range(bin_size):
        ...
        if m == bin_size - 1:
            plot[m, 0].set_xlabel(r'$\delta_{\mu_m}$')
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].axvspan(- factor_lens, + factor_lens, alpha=0.3, color='gray', rasterized=True)
        ...
        if m == bin_size - 1:
            plot[m, 1].set_xlabel(r'$\delta_{\mu_m}$')
    ...
    for m in range(bin_size):
        ...
        plot[m, 2].axvspan(- factor_source, + factor_source, alpha=0.3, color='gray', rasterized=True)
