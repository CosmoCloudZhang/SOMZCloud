...
def main(tag, label, folder):
    ...
    for m in range(bin_size):
        ...
        plot[m, 0].fill_betweenx(y=[10, 1000], x1=target_width_lens[m] - factor_lens[m], x2=target_width_lens[m] + factor_lens[m], color='gray', alpha=0.5)
        
        plot[m, 0].text(x=target_width_lens[m] + range_lens[m] / 3, y=500, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + 1), color='black')
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].hist(target_deviation_lens[:, m + bin_size], bins=size, range=(target_width_lens[m + bin_size] - range_lens[m + bin_size], target_width_lens[m + bin_size] + range_lens[m + bin_size]), color='black', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(histogram_deviation_lens[:, m + bin_size], bins=size, range=(target_width_lens[m + bin_size] - range_lens[m + bin_size], target_width_lens[m + bin_size] + range_lens[m + bin_size]), color='darkblue', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(model_deviation_lens[:, m + bin_size], bins=size, range=(target_width_lens[m + bin_size] - range_lens[m + bin_size], target_width_lens[m + bin_size] + range_lens[m + bin_size]), color='darkgreen', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].hist(product_deviation_lens[:, m + bin_size], bins=size, range=(target_width_lens[m + bin_size] - range_lens[m + bin_size], target_width_lens[m + bin_size] + range_lens[m + bin_size]), color='darkorange', density=True, histtype='step', linewidth=2.0, linestyle='-')
        
        plot[m, 1].fill_betweenx(y=[10, 1000], x1=target_width_lens[m + bin_size] - factor_lens[m + bin_size], x2=target_width_lens[m + bin_size] + factor_lens[m + bin_size], color='gray', alpha=0.5)
        
        plot[m, 1].text(x=target_width_lens[m + bin_size] + range_lens[m + bin_size] / 3, y=500, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + bin_size + 1), color='black')
    ...
    for m in range(bin_size):
        ...
        plot[m, 2].fill_betweenx(y=[4, 400], x1=target_width_source[m] - factor_source[m], x2=target_width_source[m] + factor_source[m], color='gray', alpha=0.5)
        
        plot[m, 2].text(x=target_width_source[m] + range_source[m] / 3, y=200, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + 1), color='black')
