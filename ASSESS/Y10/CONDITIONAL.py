...
def main(tag, name, index, folder):
    ...
    # Center
    center_lens = integrate.trapezoid(x=z_grid, y=z_grid[numpy.newaxis, :] * truth_lens, axis=1)
    center_source = integrate.trapezoid(x=z_grid, y=z_grid[numpy.newaxis, :] * truth_source, axis=1)
    ...
    for m in range(bin_size):
        plot[m, 1].plot(z_grid, dir_lens[m + bin_size, :], color='darkmagenta', linewidth=1.5, linestyle='-', label=r'$\mathtt{DIR}$', rasterized=True)
        
        plot[m, 1].plot(z_grid, stack_lens[m + bin_size, :], color='darkgreen', linewidth=1.5, linestyle='-', label=r'$\mathtt{Stack}$', rasterized=True)
        
        plot[m, 1].plot(z_grid, hybrid_lens[m + bin_size, :], color='darkorange', linewidth=1.5, linestyle='-', label=r'$\mathtt{Hybrid}$', rasterized=True)
        
        plot[m, 1].plot(z_grid, truth_lens[m + bin_size, :], color='black', linewidth=1.5, linestyle='-', label=r'$\mathtt{Truth}$', rasterized=True)
    ...
    for m in range(bin_size):
        plot[m, 2].plot(z_grid, dir_source[m, :], color='darkmagenta', linewidth=1.5, linestyle='-', label=r'$\mathtt{DIR}$', rasterized=True)
        
        plot[m, 2].plot(z_grid, stack_source[m, :], color='darkgreen', linewidth=1.5, linestyle='-', label=r'$\mathtt{Stack}$', rasterized=True)
        
        plot[m, 2].plot(z_grid, hybrid_source[m, :], color='darkorange', linewidth=1.5, linestyle='-', label=r'$\mathtt{Hybrid}$', rasterized=True)
        
        plot[m, 2].plot(z_grid, truth_source[m, :], color='black', linewidth=1.5, linestyle='-', label=r'$\mathtt{Truth}$', rasterized=True)
    ...
    figure.savefig(os.path.join(assess_folder, '{}/CONDITIONAL/{}/FIGURE{}.pdf'.format(tag, name, index)), format='pdf', bbox_inches='tight', dpi=512)
