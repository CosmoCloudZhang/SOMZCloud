...
def main(tag, name, number, folder):
    ...
    for m in range(bin_size):
        ...
        plot[m, 1].text(x=range_lens[m + bin_size] / 3 * 2, y=2.5, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + bin_size + 1), color='black', ha='center')
    ...
    for m in range(bin_size):
        ...
        plot[m, 2].text(x=range_source[m] / 3 * 2, y=2.5, s=r'$\mathrm{Bin \,}' + r'{:.0f}$'.format(m + 1), color='black', ha='center')
        ...
        if m == bin_size - 1:
            plot[m, 2].set_xlabel(r'$\delta_\mu$')
