...
def main(tag, index, folder):
    ...
    # Target
    with h5py.File(os.path.join(comparison_folder, '{}/TARGET/DATA{}.hdf5'.format(tag, index)), 'r') as file:
        z_phot = file['z_phot'][...]
    ...
    for m in range(mean_size):
        mean_lens = (z_mean[m] <= z_true_lens) & (z_true_lens < z_mean[m + 1])
        if numpy.sum(mean_lens) > 0:
            delta_mean_lens[m] = numpy.median(delta_lens[mean_lens])
        
        mean_source = (z_mean[m] <= z_true_source) & (z_true_source < z_mean[m + 1])
        if numpy.sum(mean_source) > 0:
            delta_mean_source[m] = numpy.median(delta_source[mean_source])
    ...
    # Save
    figure.savefig(os.path.join(figure_folder, '{}/CONTROL/FIGURE{}.pdf'.format(tag, index)), format='pdf', bbox_inches='tight', dpi=512)
    pyplot.close(figure)
